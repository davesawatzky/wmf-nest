...
import { Teacher, TeacherPayload } from '../entities/teacher.entity.js'
...
describe('Teacher E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await globalThis.prisma.tbl_user.deleteMany({
      where: {
        OR: [{ email: { startsWith: 'test_teacher_' } }, { firstName: 'TestTeacher' }],
      },
    })
  ...
  describe('Teacher Queries', () => {
    it('Should list all private teachers for both roles', async () => {
      const results = await testWithBothRoles('list private teachers', async (role) => {
        ...
        return {
          ...
          containsTestTeacher: response.data.teachers?.some((t: Teacher) => t.firstName === 'TestTeacher'),
          hasUnlistedTeacher: response.data.teachers?.some((t: Teacher) => t.lastName === 'Unlisted'),
        }
      })
    ...
    it('Should list all school teachers for both roles', async () => {
      const results = await testWithBothRoles('list school teachers', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetTeachers($teacherType: String!) {
              teachers(teacherType: $teacherType) {
    ...
    it('Should find specific teacher by ID for both roles', async () => {
      const results = await testWithBothRoles('find teacher by ID', async (role) => {
    ...
    it('Should find specific teacher by email for both roles', async () => {
      const results = await testWithBothRoles('find teacher by email', async (role) => {
        const teacherEmail =
          role === 'admin' ? 'test_teacher_admin_private@test.com' : 'test_teacher_user_private@test.com'
    ...
    it('Should return null for non-existent teacher', async () => {
      const results = await testWithBothRoles('find non-existent teacher', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetTeacher($teacherID: Int) {
              teacher(teacherID: $teacherID) {
    ...
    it('Should require teacher type for teachers query', async () => {
      const results = await testWithBothRoles('teachers query without type', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetTeachers($teacherType: String!) {
              teachers(teacherType: $teacherType) {
    ...
    it('Should return correct data types for teacher fields', async () => {
      ...
      const firstTeacher = response.data.teachers.find((t) => t.firstName === 'TestTeacher')
  ...
  describe('Teacher Create Tests', () => {
    it('Should enforce create authorization: both roles can create', async () => {
      const results = await testWithBothRoles('create teacher', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation CreateTeacher($privateTeacher: Boolean!, $schoolTeacher: Boolean!, $teacherInput: TeacherInput!) {
              teacherCreate(
    ...
    it('Should create school teacher for both roles', async () => {
      const results = await testWithBothRoles('create school teacher', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation CreateTeacher($privateTeacher: Boolean!, $schoolTeacher: Boolean!, $teacherInput: TeacherInput!) {
              teacherCreate(
    ...
    it('Should require at least one teacher type', async () => {
      const results = await testWithBothRoles('create without teacher type', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation CreateTeacher($privateTeacher: Boolean!, $schoolTeacher: Boolean!, $teacherInput: TeacherInput!) {
              teacherCreate(
        ...
        return {
          ...
          hasTypeError: response.data?.teacherCreate?.userErrors?.some((e) =>
            e.message.includes('Must specify either private teacher or school teacher'),
          ),
        }
      })
    ...
    it('Should handle duplicate email error', async () => {
      const results = await testWithBothRoles('create with duplicate email', async (role) => {
        const existingEmail =
          role === 'admin' ? 'test_teacher_admin_private@test.com' : 'test_teacher_user_private@test.com'
        ...
        return {
          userErrors: response.data?.teacherCreate?.userErrors || [],
          teacher: response.data?.teacherCreate?.teacher,
          hasEmailError: response.data?.teacherCreate?.userErrors?.some((e) =>
            e.message.includes('Email address is already taken'),
          ),
        }
      })
  ...
  describe('Teacher Update Tests', () => {
    ...
    afterAll(async () => {
      ...
      const idsToDelete = [updateTestAdminTeacherId, updateTestUserTeacherId].filter((id) => id !== undefined)
    ...
    it('Should successfully update teacher for both roles', async () => {
      const results = await testWithBothRoles('update teacher', async (role) => {
        const teacherId = role === 'admin' ? updateTestAdminTeacherId : updateTestUserTeacherId

        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation UpdateTeacher($teacherID: Int!, $teacherInput: TeacherInput!) {
              teacherUpdate(teacherID: $teacherID, teacherInput: $teacherInput) {
                userErrors {
                  message
                  field
                }
    ...
    it('Should update partial fields for both roles', async () => {
      const results = await testWithBothRoles('update partial fields', async (role) => {
        const teacherId = role === 'admin' ? updateTestAdminTeacherId : updateTestUserTeacherId
    ...
    it('Should handle update with invalid teacher ID', async () => {
      const results = await testWithBothRoles('update non-existent teacher', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation UpdateTeacher($teacherID: Int!, $teacherInput: TeacherInput!) {
              teacherUpdate(teacherID: $teacherID, teacherInput: $teacherInput) {
        ...
        return {
          ...
          hasNotFoundError: response.data?.teacherUpdate?.userErrors?.some((e) =>
            e.message.includes('Teacher not found'),
          ),
        }
      })
  ...
  describe('Teacher Delete Tests', () => {
    it('Should enforce delete authorization: both roles can delete', async () => {
      const results = await testWithBothRoles('delete teacher', async (role) => {
    ...
    it('Should handle delete of non-existent teacher', async () => {
      const results = await testWithBothRoles('delete non-existent teacher', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation DeleteTeacher($teacherID: Int!) {
              teacherDelete(teacherID: $teacherID) {
        ...
        return {
          ...
          hasNotFoundError: response.data?.teacherDelete?.userErrors?.some((e) =>
            e.message.includes('Teacher not found'),
          ),
        }
      })
  ...
  describe('Teacher Field Validation', () => {
    it('Should handle all optional fields correctly', async () => {
      const results = await testWithBothRoles('create with all fields', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .mutate(gql`
            mutation CreateTeacher($privateTeacher: Boolean!, $schoolTeacher: Boolean!, $teacherInput: TeacherInput!) {
              teacherCreate(
