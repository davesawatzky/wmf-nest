...
describe('Subdiscipline E2E Tests', () => {
  ...
  describe('Subdiscipline Queries (Both Roles)', () => {
    it('Should list all subdisciplines for both roles', async () => {
      const results = await testWithBothRoles('list subdisciplines', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetSubdisciplines($performerType: PerformerType, $disciplineId: Int) {
        ...
        return {
          ...
          hasValidTypes: typeof firstSubdiscipline?.id === 'number' && typeof firstSubdiscipline?.name === 'string',
        }
      })
    ...
    it('Should filter subdisciplines by disciplineID for both roles', async () => {
      const results = await testWithBothRoles('filter by disciplineID', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetSubdisciplines($performerType: PerformerType, $disciplineId: Int) {
              subdisciplines(performerType: $performerType, disciplineID: $disciplineId) {
    ...
    it('Should filter subdisciplines by both performerType and disciplineID for both roles', async () => {
      const results = await testWithBothRoles('filter by both parameters', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetSubdisciplines($performerType: PerformerType, $disciplineId: Int) {
              subdisciplines(performerType: $performerType, disciplineID: $disciplineId) {
        ...
        return {
          hasData: !!response.data.subdisciplines,
          count: response.data.subdisciplines?.length || 0,
          hasGuitarSubdiscipline: response.data.subdisciplines.some((s) => s.name.includes('GUITAR')),
        }
      })
    ...
    it('Should list subdisciplines with associated categories for both roles', async () => {
      const results = await testWithBothRoles('list with categories', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
    ...
    it('Should list subdisciplines with associated levels for both roles', async () => {
      const results = await testWithBothRoles('list with levels', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetSubdisciplines {
              subdisciplines {
        ...
        const subdisciplineWithLevels = response.data.subdisciplines.find((s) => s.levels && s.levels.length > 0)
    ...
    it('Should list subdisciplines with associated discipline for both roles', async () => {
      const results = await testWithBothRoles('list with discipline', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
    ...
    it('Should find specific subdiscipline by ID for both roles', async () => {
      const results = await testWithBothRoles('find subdiscipline by ID', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
    ...
    it('Should return error when subdiscipline not found for both roles', async () => {
      const results = await testWithBothRoles('subdiscipline not found', async (role) => {
        const response = (await createAuthenticatedRequest(role)
          .query(gql`
            query GetSubdiscipline($subdisciplineId: Int!) {
              subdiscipline(subdisciplineID: $subdisciplineId) {
  ...
  describe('Subdiscipline Mutations', () => {
    it('Should enforce create authorization: admin succeeds, user fails', async () => {
      const results = await testWithBothRoles('create subdiscipline', async (role) => {
    ...
    it('Should enforce update authorization: admin succeeds, user fails', async () => {
      ...
      const results = await testWithBothRoles('update subdiscipline', async (role) => {
        const response = (await createAuthenticatedRequest(role).mutate(
          gql`
            mutation UpdateSubdiscipline($subdisciplineId: Int!, $subdisciplineInput: SubdisciplineInput!) {
    ...
    it('Should enforce delete authorization: admin succeeds, user fails', async () => {
      ...
      const results = await testWithBothRoles('delete subdiscipline', async (role) => {
