...
export class FestivalClassDataLoader {
  ...
  // Batch load levels
  readonly levelLoader = new DataLoader<number, tbl_level | null>(async (levelIDs: readonly number[]) => {
  ...
  readonly subdisciplineLoader = new DataLoader<number, tbl_subdiscipline | null>(
    async (subdisciplineIDs: readonly number[]) => {
      ...
      const subdisciplineMap = new Map(subdisciplines.map((subdiscipline) => [subdiscipline.id, subdiscipline]))
      return subdisciplineIDs.map((id) => subdisciplineMap.get(id) || null)
    },
  )

  // Batch load categories
  readonly categoryLoader = new DataLoader<number, tbl_category | null>(async (categoryIDs: readonly number[]) => {
  ...
  readonly classTypeLoader = new DataLoader<number, tbl_class_type | null>(async (classTypeIDs: readonly number[]) => {
