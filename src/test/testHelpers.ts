...
export function getAuthToken(role: UserRole): string {
  ...
  switch (role) {
    case 'admin':
      return context.admin.token
    case 'user':
      return context.user.token
    case 'privateTeacher':
      return context.privateTeacher?.token || context.user.token
    case 'schoolTeacher':
      return context.schoolTeacher?.token || context.user.token
    default:
...
export function getUserId(role: UserRole): number {
  ...
  switch (role) {
    ...
    case 'privateTeacher':
      return context.privateTeacher?.userId || context.user.userId
    case 'schoolTeacher':
      return context.schoolTeacher?.userId || context.user.userId
    default:
      throw new Error(`Unknown role: ${role}`)
  }
}
...
export function createAuthenticatedRequest<T>(role: UserRole) {
  ...
  return request<T>(globalThis.httpServer).set('Cookie', `diatonicToken=${token}`)
}
...
export async function expectAuthorized<T>(role: UserRole, operation: () => Promise<T | any>): Promise<any> {
...
export function testAsRoles(roles: UserRole[], testFn: (role: UserRole) => void | Promise<void>) {
